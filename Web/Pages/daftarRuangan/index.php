...
<script>
    ...
    try {
        ...
        if (result.status === "success") {
            ...
            // Filter ruangan berdasarkan search query (jika ada)
            const searchedRooms = searchQuery 
                ? rooms.filter(room => room.nama_ruangan.toLowerCase().includes(searchQuery.toLowerCase()) || room.nama_gedung.toLowerCase().includes(searchQuery.toLowerCase())) 
                : rooms;

            roomList.innerHTML = ''; // Hapus isi room list sebelum render ulang

            // Render setiap ruangan hasil pencarian
