    ...
    <div class="flex items-center justify-between max-w-screen-xl mx-auto gap-x-3 flex-wrap md:flex-nowrap">
        ...
        <div class="flex-grow max-w-md mx-auto" id="searchForm">
            <form class="flex flex-row gap-2 items-center">
                ...
                <button type="submit" class="right-2 top-1/2 bg-blue-800 text-white px-4 py-1 rounded-md h-10">
                    Cari
                </button>
            </form>
        </div>
        ...
        <div class="ml-auto p-5 flex  cursor-pointer items-center space-x-3 flex items-center p-2 group hover:text-white text-gray-400 <?php echo $current_page == 'Profile' ? 'active bg-biru-500 text-white' : ''; ?>"
            onclick="window.location.href='../Profile/index.php'">
            ...
            <span class="text-sm md:text-base" id="nama_lengkap"></span>
        </div>
    </div>
</div>
...
document.querySelector('form').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (data.status === "success") {
            ...
            const gedungMatch = data.data.find(ruangan => ruangan.nama_gedung.toLowerCase() ===
                searchQuery);

            // Cari kecocokan berdasarkan nama ruangan
            const ruanganMatch = data.data.filter(ruangan => ruangan.nama_ruangan.toLowerCase() ===
                searchQuery);
            ...
            if (gedungMatch) {
                ...
                window.location.href =
                    `http://localhost/sipinjamfix/sipinjam/Web/Pages/daftarRuangan/?id_gedung=${gedungMatch.id_gedung}`;
            } else if (ruanganMatch.length > 0) {
                // Redirect ke halaman daftarRuangan dengan query nama_ruangan jika ruangan ditemukan
                window.location.href =
                    `http://localhost/sipinjamfix/sipinjam/Web/Pages/daftarRuangan/?search=${encodeURIComponent(searchQuery)}`;
            } else {
